  ...
  private static final int MIN_TEL = 24;
   ...
   public static void main(String[] args) throws Exception {     
      ...
      while ((line = bf.readLine()) != null) {
          ...
          BitSet b = scaffolds.get(split[0]);
          int telomereLength = Integer.parseInt(split[5]);
          if (telomereLength >= MIN_TEL) {
                if (telomereLength >= 100) {
                      int start = Math.max(0, Integer.parseInt(split[3])-50);
                      int end = Math.min(lengths.get(split[0]), Integer.parseInt(split[4])+50);
                      if (b.get(start, end).cardinality() >= (end - start) * 0.5) {
                              finalScaffolds.get(split[0]).set(
